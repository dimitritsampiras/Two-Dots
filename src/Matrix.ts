...
export class Matrix<T> {
  ...
  public constructor(M: Array<Array<T>>) {
    if (!M.length) throw new Error("Illegal Argument: Grid is empty");
    if (!M[0].length) throw new Error("Illegal Argument: Grid is empty");
    this.width = M.length;
  ...
  public get(p: PointT): T {
    if (!this.validPoint(p)) throw new Error("Invalid point");
